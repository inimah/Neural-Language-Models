...
if __name__ == '__main__':
	...
	model = classificationModelBiLSTMDense(MAX_SEQUENCE_LENGTH, VOCAB_LENGTH, numClasses, EMBEDDING_DIM, w2v_subj_embed1)
	model.fit(X, yCategorical, batch_size=BATCH_SIZE, nb_epoch=NB_EPOCH, callbacks=[history])

	model.save('subj_CM2b.h5')
	model.save_weights('subj_weights_CM2b.hdf5')
	savePickle(history.losses,'subj_CM2b_loss')
	savePickle(history.acc,'subj_CM2b_accuracy')

	# embedding layer
	embedSubj = Model(inputs=model.input, outputs=model.get_layer('embedding_layer').output)
	sent_embed_CM2b = embedSubj.predict(X)
	#savePickle(sent_embed_CM2b,'sent_embed_CM2b')
	saveH5File('sent_embed_CM2b.h5','sent_embed_CM2b',sent_embed_CM2b)

	# encoder layer
	encoderSubj = Model(inputs=model.input, outputs=model.get_layer('bilstm_encoder').output)
	doc_embed_CM2b = encoderSubj.predict(X)
	#savePickle(doc_embed_CM2b,'doc_embed_CM2b')
	saveH5File('doc_embed_CM2b.h5','doc_embed_CM2b',doc_embed_CM2b)

	# output layer
	output_pred = model.predict(X)
	saveH5File('embed_pred_CM2b.h5','output_pred',output_pred)