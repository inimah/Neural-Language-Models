...
if __name__ == '__main__':
	...
	model = languageModel4(MAX_SEQUENCE_LENGTH, VOCAB_LENGTH, EMBEDDING_DIM, w2v_subj_embed1)
	model.fit(X, Y_encoded, batch_size=BATCH_SIZE, nb_epoch=NB_EPOCH, callbacks=[history])

	model.save('subj_LM2a.h5')
	model.save_weights('subj_weights_LM2a.hdf5')
	savePickle(history.losses,'subj_LM2a_loss')
	savePickle(history.acc,'subj_LM2a_accuracy')



	# embedding layer
	embedSubj = Model(inputs=model.input, outputs=model.get_layer('embedding_layer').output)
	word_embed_LM2a = embedSubj.predict(X)
	savePickle(word_embed_LM2a,'word_embed_LM2a')
	saveH5File('word_embed_LM2a.h5','word_embed_LM2a',word_embed_LM2a)

	# encoder layer
	encoderSubj = Model(inputs=model.input, outputs=model.get_layer('lstm_enc').output)
	doc_embed_LM2a = encoderSubj.predict(X)
	savePickle(doc_embed_LM2a,'doc_embed_LM2a')
	saveH5File('doc_embed_LM2a.h5','doc_embed_LM2a',doc_embed_LM2a)

	# output layer
	output_pred = model.predict(X)
	savePickle(output_pred,'embed_pred_LM2a')
	saveH5File('embed_pred_LM2a.h5','output_pred',output_pred)
