...
if __name__ == '__main__':
	...
	model = languageModelBiGRU(MAX_SEQUENCE_LENGTH, VOCAB_LENGTH, EMBEDDING_DIM, w2v_subj_embed1)
	model.fit(X, Y_encoded, batch_size=BATCH_SIZE, nb_epoch=NB_EPOCH, callbacks=[history])

	model.save('subj_LM1d.h5')
	model.save_weights('subj_weights_LM1d.hdf5')
	savePickle(history.losses,'subj_LM1d_loss')
	savePickle(history.acc,'subj_LM1d_accuracy')

	# embedding layer
	embedSubj = Model(inputs=model.input, outputs=model.get_layer('embedding_layer').output)
	sent_embed_LM1d = embedSubj.predict(X)
	#savePickle(sent_embed_LM1d,'sent_embed_LM1d')
	saveH5File('sent_embed_LM1d.h5','sent_embed_LM1d',sent_embed_LM1d)

	# encoder layer
	encoderSubj = Model(inputs=model.input, outputs=model.get_layer('bigru_encoder').output)
	doc_embed_LM1d = encoderSubj.predict(X)
	#savePickle(doc_embed_LM1d,'doc_embed_LM1d')
	saveH5File('doc_embed_LM1d.h5','doc_embed_LM1d',doc_embed_LM1d)

	# decoder
	decoderSubj = Model(inputs=model.input, outputs=model.get_layer('bigru_decoder_2').output)
	doc_sent_embed_LM1d = decoderSubj.predict(X)
	#savePickle(doc_sent_embed_LM1d,'doc_sent_embed_LM1d')
	saveH5File('doc_sent_embed_LM1d.h5','doc_sent_embed_LM1d',doc_sent_embed_LM1d)


	# output layer
	output_pred = model.predict(X)
	#savePickle(output_pred,'embed_pred_LM1d')
	saveH5File('embed_pred_LM1d.h5','output_pred',output_pred)
