...
if __name__ == '__main__':
	...
	model = languageModel5(MAX_SEQUENCE_LENGTH, VOCAB_LENGTH, EMBEDDING_DIM, w2v_subj_embed1)
	model.fit(X, Y_encoded, batch_size=BATCH_SIZE, nb_epoch=NB_EPOCH, callbacks=[history])

	model.save('subj_LM2b.h5')
	model.save_weights('subj_weights_LM2b.hdf5')
	savePickle(history.losses,'subj_LM2b_loss')
	savePickle(history.acc,'subj_LM2b_accuracy')



	# embedding layer
	embedSubj = Model(inputs=model.input, outputs=model.get_layer('embedding_layer').output)
	word_embed_LM2b = embedSubj.predict(X)
	savePickle(word_embed_LM2b,'word_embed_LM2b')
	saveH5File('word_embed_LM2b.h5','word_embed_LM2b',word_embed_LM2b)

	# encoder layer
	encoderSubj = Model(inputs=model.input, outputs=model.get_layer('bilstm_enc').output)
	doc_embed_LM2b = encoderSubj.predict(X)
	savePickle(doc_embed_LM2b,'doc_embed_LM2b')
	saveH5File('doc_embed_LM2b.h5','doc_embed_LM2b',doc_embed_LM2b)

	# output layer
	output_pred = model.predict(X)
	savePickle(output_pred,'embed_pred_LM2b')
	saveH5File('embed_pred_LM2b.h5','output_pred',output_pred)
