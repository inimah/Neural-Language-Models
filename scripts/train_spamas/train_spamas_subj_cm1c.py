...
if __name__ == '__main__':
	...
	model = classificationModelGRUDense(MAX_SEQUENCE_LENGTH, VOCAB_LENGTH, numClasses, EMBEDDING_DIM, w2v_subj_embed1)
	model.fit(X, yCategorical, batch_size=BATCH_SIZE, nb_epoch=NB_EPOCH, callbacks=[history])

	model.save('subj_CM1c.h5')
	model.save_weights('subj_weights_CM1c.hdf5')
	savePickle(history.losses,'subj_CM1c_loss')
	savePickle(history.acc,'subj_CM1c_accuracy')

	# embedding layer
	embedSubj = Model(inputs=model.input, outputs=model.get_layer('embedding_layer').output)
	sent_embed_CM1c = embedSubj.predict(X)
	#savePickle(sent_embed_CM1c,'sent_embed_CM1c')
	saveH5File('sent_embed_CM1c.h5','sent_embed_CM1c',sent_embed_CM1c)

	# encoder layer
	encoderSubj = Model(inputs=model.input, outputs=model.get_layer('gru_encoder').output)
	doc_embed_CM1c = encoderSubj.predict(X)
	#savePickle(doc_embed_CM1c,'doc_embed_CM1c')
	saveH5File('doc_embed_CM1c.h5','doc_embed_CM1c',doc_embed_CM1c)

	# output layer
	output_pred = model.predict(X)
	saveH5File('embed_pred_CM1c.h5','output_pred',output_pred)