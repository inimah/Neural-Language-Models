...
if __name__ == '__main__':
	...
	model.fit(X_samples3, yEncoded, batch_size=BATCH_SIZE, nb_epoch=NB_EPOCH, callbacks=[history])

	model.save('ls_cont3_CM.h5')
	model.save_weights('ls_cont3_weights_CM.hdf5')
	savePickle(history.losses,'ls_cont3_CM_history.losses')
	savePickle(history.acc,'ls_cont3_CM_history.acc')

	encoderSubj = Model(inputs=model.input, outputs=model.get_layer('lstm_enc').output)
	encoded_subj = encoderSubj.predict(X)
	savePickle(encoded_subj,'ls_cont3_encoded_CM')
